	...
	public partial class VoxelPlayFirstPersonController : VoxelPlayCharacterControllerBase {
		...
		private void FixedUpdate () {
			...
			if (canMove) {
                ...
                //m_MoveDir.y = 0;
                //Debug.Log(m_MoveDir);
                //m_CharacterController.transform.parent.RotateAround(Vector3.zero, m_MoveDir, 40 * Time.fixedDeltaTime);
                //Vector3 v = m_CharacterController.transform.parent.eulerAngles;
                //v += m_MoveDir;
                //m_CharacterController.transform.parent.eulerAngles = v;
		...
		private void GetInput (out float speed) {
            ...
            if (isMouseRotation) {
                if (Input.GetMouseButton(1)){
                    mouseRotateValue += Input.GetAxisRaw("Mouse X");
                }
                else {
                    mouseRotateValue = 0;
                }
                    
                m_Input = new Vector3(mouseRotateValue, input.verticalAxis, up);
                speed *= 0.7f;
            }
            else {
