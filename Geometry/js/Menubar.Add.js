...
Menubar.Add = function ( editor ) {
	...
	option.onClick( function () {
		...
		// var dotMaterial = new THREE.PointsMaterial( { size: 10, sizeAttenuation: false, map: sprite, transparent: true } );
		// var dot = new THREE.Points( dotGeometry, dotMaterial );
		// dot.name = 'Point'
		// editor.execute( new AddObjectCommand( dot ) );
		
		// let dot = new Algeomath.object.GenericPoint(new THREE.Vector3( 0, 0, 0), "dot");		
		// editor.execute( new AddObjectCommand( dot ) );
		editor.tool = new Algeomath.tool.GenericPoint()

		// editor.execute( new MoveObjectCommand( dot, mesh, null ) );
	} );
	options.add( option );
	...
	option.onClick( function () {
		...
		// line.name = "Line"
		// editor.execute( new AddObjectCommand( line ) );		
		editor.tool = new Algeomath.tool.GenericSegment()
...
// 0   1   2   3   4   5
		...
		// editor.execute( new AddObjectCommand( zenithEquator ) );
	...
	option.onClick( function () {
		...
		// var dotGeometry = new THREE.Geometry();
		// dotGeometry.vertices.push(new THREE.Vector3( 0, 0, 0));
		// dotGeometry.vertices.push(new THREE.Vector3( 0.5, 0.5, 0.5));
		// dotGeometry.vertices.push(new THREE.Vector3( -0.5, 0.5, 0.5));
		// dotGeometry.vertices.push(new THREE.Vector3( 0.5, -0.5, 0.5));
		// dotGeometry.vertices.push(new THREE.Vector3( -0.5, -0.5, 0.5));		
		// var dotMaterial = new THREE.PointsMaterial( { size: 10, sizeAttenuation: false, map: sprite, transparent: true } );
