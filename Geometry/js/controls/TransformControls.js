...
THREE.TransformControls = function ( camera, domElement ) {
	...
	this.pointerMove = function( pointer ) {
		...
		if ( mode === 'translate' ) {
			...
			if (object instanceof Algeomath.object.GenericSegment){
				object.move(positionStart.sub(object.position));
				object.updateGeometry()
			}
			if(null != object.dependers){
				for (let d of object.dependers) {
					if(d instanceof Algeomath.object.GenericSegment)
						d.updateGeometry()
				}
			}
		} else if ( mode === 'scale' ) {
...
THREE.TransformControlsGizmo = function () {
	...
	this.updateMatrixWorld = function () {
		...
		for ( var i = 0; i < handles.length; i++ ) {
			...
			// eyeDistance = 0 >= eyeDistance ? 1 : eyeDistance;
